...
can run `scry-web` against something live without setting up a real
...
Then in `scry-web` open Settings → Daemon URL, set it to
...
`~/.scry/maps/` if you have a scry install locally.
...
clamped to `1..5000`). Useful for poking at scry-web rendering perf
